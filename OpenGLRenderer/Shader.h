...
public:
	...
	Shader(const char* vertexPath, const char* fragmentPath) 
	{
		...
		// 2. compile shader and log errors
...
private:
	...
	void checkUniformExists(const std::string& name) const
	{
		GLint uniformLoc = glGetUniformLocation(ID, name.c_str());
		
		if (uniformLoc == -1)
		{
			std::cout << "WARNING: uniform " << name << " not found in shader with ID " << ID << std::endl;
		}
	}
};